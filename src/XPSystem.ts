 ...
 * (level, progress, etc.) is derived on-the-fly from that number.
 *
 * @example
 * ```ts
...
export class XPSystem {
   ...
   */
  addXP(currentTotalXP: number, amount: number): XPChangeResult {
    ...
    const newTotalXP = Math.min(currentTotalXP + amount, maxXP);
   ...
   */
  removeXP(currentTotalXP: number, amount: number): XPChangeResult {
